...
function check_collisions(pacman_differences, ghost_differences){
  ...
  */
  ...
  let ghosts_collided_with_scared = []
  let ghosts_collided_with_not_scared = []
  for(var ghost_diff_idx in ghosts){
    ...
    if(ghost_x_cord === pacman_x_cord && ghost_y_cord === pacman_y_cord){
      if(ghost.scared){
        ghosts_collided_with_scared.push(ghost);
      }
      else {
        ghosts_collided_with_not_scared.push(ghost)
      }
    } 
  }
  return {'scared': ghosts_collided_with_scared, 'not_scared':ghosts_collided_with_not_scared}
}
...
function frame(){
  ...
  if(frame_number % 3){ // only move every 3 frames, so 4 moving frames per second
    ...
    let ghosts_collided_with = check_collisions(pacman_differences, ghost_differences)
    if(ghosts_collided_with['not_scared'].length){
      document.getElementById("status").innerHTML = "🙁 Game Over 🙁";
      pacman_differences = pacman_differences.slice(0, 1); // don't render pacman's new position -- TODO: need to add animation
      clearInterval(interval);
    } 
    else if(ghosts_collided_with['scared'].length){
      ghosts_collided_with['scared'].forEach((ghost, idx) => {
        ghost.eaten = true;
        score += 100;
      })
    }
