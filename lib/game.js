function no_coins_left(){
  for(let i = 0 ; i < board.tile_map.length - 1 ; i++){
    for(let j = 0; j < board.tile_map[0].length - 1 ; j++){
      if(board.tile_map[i][j].has_coin) {
        return false
      }
    }
  }
...
function check_collisions(pacman_differences, ghost_differences){
  /*
    ...
    BE CAREFUL IF YOU STOP RED FROM MOVING, THIS ASSUMES RED WILL ALWAYS HAVE DIFFERENCES IF PINK DOES
    THEN ASSUMES THAT PINK HAS DIFFERENCES IF BLUE DOES, AND BLUE HAS IF ORANGES DOES
  */
  let pacman_x_cord = pacman.x_cord;
  let pacman_y_cord = pacman.y_cord;
  let pacman_prev_x_cord = pacman.x_cord;
  let pacman_prev_y_cord = pacman.y_cord;
  if(pacman_differences.length){
    pacman_prev_x_cord = pacman_differences[0][0];
    pacman_prev_y_cord = pacman_differences[0][1];
  }
  for(var ghost_diff_idx in ghosts){
    let ghost = ghosts[ghost_diff_idx];
    // THE BELOW LINE GETS THE DIFFERENCES FOR THAT COLOR, TERRIBLE WAY TO DO IT, AS IT ASSUMES
    // THE ABOVE
    this_ghost_differences = ghost_differences.slice(ghost_diff_idx * 2, ghost_diff_idx * 2 + 2)
    ...
    if(this_ghost_differences.length){
      let ghost_prev_x_cord = this_ghost_differences[0][0];
      let ghost_prev_y_cord = this_ghost_differences[0][1];
      if(ghost_prev_x_cord === pacman_x_cord && ghost_prev_y_cord === pacman_y_cord &&
         ghost_x_cord === pacman_prev_x_cord && ghost_y_cord === pacman_prev_y_cord ){ // they swapped places == they walked past each other
        return true
      }
    }
  }
...
function frame(){
  ...
  if(frame_number % 3){
    ...
    if(check_collisions(pacman_differences, ghost_differences)){
...
interval = setInterval(frame, 1000/10) // divide by 9, because they only move once every 9 frames
